...
#define	f1_as_plier	(*(__uint24 *)&f1)
#define	f2_as_plicand	(*(__uint24 *)&f2)

// floating addition
#ifdef _OLDLIB
#define	f3_as_product		(*(__uint24 *)&f3)
float
...
__ftmul(float f1, float f2)
{
	...
	__uint24	f3_as_product;

	if((exp = (unsigned char)(f1_as_plier >> 15)) == 0)
		return 0.0;
	if((exp2 = (unsigned char)(f2_as_plicand >> 15)) == 0)
		return 0.0;
