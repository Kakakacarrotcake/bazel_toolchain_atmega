...
static float erfc2(uint32_t ix, float x)
{
	...
	z.f = x;
	z.i &= UINT32_C(0xffffe000);
	return expf(-z.f*z.f - 0.5625f) * expf((z.f-x)*(z.f+x) + R/S)/x;
}
...
float erff(float x)
{
	...
	if (ux.i < 0x3f580000) {  /* |x| < 0.84375 */
		if (ux.i < 0x31800000) {  /* |x| < 2**-28 */
			/*avoid underflow */
...
float erfcf(float x)
{
	...
	if (ix.i < 0x3f580000) {  /* |x| < 0.84375 */
		if (ix.i < 0x23800000)  /* |x| < 2**-56 */
			return 1.0f - x;
		z = x*x;
		r = pp0+z*(pp1+z*(pp2+z*(pp3+z*pp4)));
		s = 1.0f+z*(qq1+z*(qq2+z*(qq3+z*(qq4+z*qq5))));
		y = r/s;
		if (sign || ix.i < 0x3e800000)  /* x < 1/4 */
			return 1.0f - (x+x*y);
		return 0.5f - (x - 0.5f + x*y);
	}
	if (ix.i < 0x41e00000) {  /* |x| < 28 */
		return sign ? 2 - erfc2(ix.i,x) : erfc2(ix.i,x);
	}
